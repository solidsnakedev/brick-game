...
import Control.Concurrent
    ( threadDelay, forkIO, newEmptyMVar, putMVar, tryTakeMVar )
import Control.Monad.State.Strict
    ( MonadTrans(lift), MonadState(put, get), StateT(runStateT) )
...
createBall n column = mconcat ["|", replicate n ' ', "*", replicate (column - n -1) ' ', "|"]

createEmptySpace :: Int -> [Char]
createEmptySpace n = mconcat ["|", replicate n ' ', "|"]

createBoard :: Int -> Int -> Int -> [Char]
createBoard n column size = mconcat ["|", replicate n ' ', replicate size '=', replicate (column - size - n) ' ', "|"]

createBox :: Int -> Int -> (Int, Int) -> [String]
createBox column row (ballPosX, ballPosY) =
  mconcat
...
getCommnads :: Char -> IO ()
getCommnads c
  | c == 'w' = putStrLn $ unlines $ createBox 10 10 (7, 4)
...
data GameState = GameState {box :: String, board :: String, boardPos :: Int, ballPos :: (Int,Int), directionY::DirectionY, directionX :: DirectionX} deriving Show
initGameSate =  GameState {box = "", board = "", boardPos = 0, ballPos = (5,5), directionY = GoUp, directionX= GoRight}

data Env = Env {column :: Int, row :: Int, boardSize :: Int} deriving Show
initEnv = Env 10 10 3
...
mainGame = do
  ...
  where wait threadVar env state = do
                      ...
                      case mVar of
                        Just var -> case var of
                                    'a' -> do
                                      ...
                                      -- update State
                                      put $ GameState {box = "", board = "", boardPos = p, ballPos = (posX', posY'), directionY = directionY', directionX = directionX'}
                        ...
                        Nothing -> do
                                  ...
                                  let box' =  unlines $ createBox (column env) (row env) (posX', posY')
                                  -- update State
                                  put $ GameState {box = "", board = "", boardPos = boardPos state, ballPos = (posX', posY'), directionY = directionY', directionX = directionX'}
...
ballGame = do
    ...
    lift $ put GameState {box = box', board = board st, boardPos = boardPos st, ballPos = (posX', posY'), directionY = directionY', directionX = directionX'}
    --lift $ lift $ putStrLn box'
    return box'
...
waiting' c = do
  ...
  case a of
    ...
    Nothing -> threadDelay 1000000 >> putStrLn "waiting for input" >> waiting' c
